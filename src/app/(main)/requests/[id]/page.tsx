    ...
    <div className="w-full flex flex-col gap-y-5 p-5">
      ...
      <div className="flex items-center justify-between gap-4 ">
        ...
        <div className="flex items-center gap-4 ">
          <Avatar>
            ...
            <AvatarFallback>{request.user.name?.[0]?.toUpperCase()}</AvatarFallback>
          </Avatar>
          <div>
            <h3 className="text-lg font-semibold">{request.user.name}</h3>
            <p className="text-sm text-gray-500">Дата отправки: {request.date}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex w-full justify-between gap-4 mt-6">
        ...
        <Dialog>
          ...
          <DialogContent>
            <DialogTitle>Отмена запроса</DialogTitle>
            <DialogDescription>Вы уверены, что хотите отклонить запрос?</DialogDescription>
            <div className="flex gap-4">
              <DialogClose asChild>
                <Button className="w-full" onClick={handleDeclineRequest}>Да, отклонить</Button>
              </DialogClose>
              <DialogClose asChild>
